...
class MailManager {
	...
	load() {
		...
		for (const filename of mailFilenames) {
			...
			Consola.success(`Loaded ${mail.name} (${recipients.length} recipients, ${Object.keys(variables).length} variables)!`);
		}
	...
	import(mails) {
		for (const mail of mails) {
			...
			Consola.success(`Loaded ${mail.name} (${mail.recipients.length} recipients, ${Object.keys(mail.variables).length} variables)!`);
		}
	}
	...
	importMail(name, mail) {
		...
		Consola.success(`Loaded ${name} (${mail.recipients.length} recipients, ${Object.keys(mail.variables).length} variables)!`);
	}
